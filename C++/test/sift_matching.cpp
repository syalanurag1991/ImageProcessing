#include "sift_matching.hpp"

using namespace cv;
using namespace cv::xfeatures2d;

void display_SIFT_features(const char *filename)
{
	...
	sprintf(figure_name, "SIFT Keypoints in %s", filename);
	imshow( figure_name, output );

	sprintf(output_filename, "%s_SIFT_Keypoints.png", filename);
...
void SIFT_feature_matching(const char *img1_filename, const char *img2_filename)
{
	...
	detector->detect(img1, kp_img1);
	detector->detect(img2, kp_img2);
	
	extractor->compute(img1, kp_img1, dsc_img1);
	extractor->compute(img2, kp_img2, dsc_img2);
	...
	sprintf(output_figurename, "TEMP");
	sprintf(output_filename, "SIFT_Match_%s_and_%s.png", img1_filename, img2_filename);
